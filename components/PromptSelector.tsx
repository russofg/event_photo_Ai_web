    ...
    <div className="space-y-4 sm:space-y-8">
      ...
      <div>
        <label className="block text-base sm:text-xl font-bold text-white mb-3 sm:mb-4 flex items-center gap-2">
          <span className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-blue-500 flex items-center justify-center text-xs sm:text-sm">1</span>
          Elige tu Escenario
        </label>
          ...
          {presets.map((preset) => {
            ...
            return (
              <button
                ...
                `}
              >
                <div className={`text-3xl sm:text-5xl mb-2 sm:mb-3 transition-transform duration-300 ${isSelected ? 'scale-110' : ''}`}>
                  {preset.icon}
                </div>
                <span className={`text-xs sm:text-lg font-bold text-center ${isSelected ? 'text-white' : 'text-gray-300'}`}>
                  {preset.label}
                </span>
                {isSelected && (
                  <div className="absolute top-2 right-2 sm:top-3 sm:right-3 w-5 h-5 sm:w-6 sm:h-6 bg-blue-500 rounded-full flex items-center justify-center">
                    <svg className="w-3 h-3 sm:w-4 sm:h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>
                  </div>
                )}
              </button>
            );
      ...
      <div>
        <label className="block text-base sm:text-xl font-bold text-white mb-3 sm:mb-4 flex items-center gap-2">
          <span className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-purple-500 flex items-center justify-center text-xs sm:text-sm">2</span>
          Elige el Estilo
        </label>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
          {ART_STYLES.map((style) => (
            <button
              ...
              `}
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${style.previewColor} opacity-20 transition-opacity`} />
              <span className={`block text-sm sm:text-lg font-bold z-10 relative ${selectedStyleId === style.id ? 'text-white' : 'text-gray-300'}`}>
                {style.label}
              </span>
            </button>
          ))}
